@extends('template.main', ['menu' => "admin", 'submenu' => "Gastos", 'rota'=>"gasto.create"])
        ...
        <table class="table align-middle caption-top table-striped bg-white table-custom">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $item->pagamento->nome ?? 'Forma de Pagamento não encontrada' }}</td>
                    <td>
                        <a href="{{ route('gasto.edit', $item->id) }}" class="btn btn-primary btn-custom">
                    ...
                    <form action="{{ route('gasto.destroy', $item->id) }}" method="POST" id="form_{{$item->id}}">
                        @csrf
                        @method('DELETE')
                    </form>
                </tr>
